...
function AudioLessonCard({ lesson, refetch }) {
  const [visible, setvisible] = useState(true)
  const {
    mutate: deleteAudioLesson,
    isLoading,
  } = useMutation(
    () => trainerPackage.deleteAudioLesson({
      trainerPackageId: lesson.trainerPackageId,
      audioLessonId: lesson.id,
      key: lesson.key
    }), {
    onSuccess: () => {
      console.log('good')
  ...
  return (
    <div className="relative border p-4 rounded-lg shadow-lg" style={{ display: visible ? 'block' : 'none' }}>
      ...
      <p className="mt-2">
        Ноты:{" "}
        <a className="text-blue-500 underline break-words" href={lesson.notesUrl} target="blank">
          {lesson.notesUrl}
        </a>
      </p>
      <img
